...
// MEJORA 4: Preview de foto del huésped
...
// MEJORA 4: Guardar reserva con campos simplificados y foto
document.getElementById('step3-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // ✅ Guardar huésped (SIN teléfono, documento, etc.)
        const [{ data: guest, error: guestError }, receiptUrl] = await Promise.all([
            db.from('guests').insert([{
                ...
                // ELIMINADO: phone, document_type, document_id
            }]).select().single(),
...
async function showReservationDetail(reservationId) {
    ...
    // MEJORA 5: Mostrar foto del huésped si existe
    ...
    // MEJORA 5: Botones de acción mejorados
    const actions = document.getElementById('detail-actions');
    actions.innerHTML = '';
    
    // Botón Editar (siempre visible excepto cancelada/check-out)
...
// MEJORA 5: Abrir modal de edición
async function openEditReservation(reservationId) {
    ...
    // Cargar opciones de habitación/cama
    ...
    // Otras habitaciones privadas
    rooms.filter(r => r.type === 'private').forEach(room => {
...
// MEJORA 5: Cancelar con confirmación y revertir pagos en efectivo
async function cancelReservation(reservationId) {
    ...
    try {
        ...
        // Calcular reembolsos en efectivo
        ...
        // Registrar reembolso en efectivo si aplica

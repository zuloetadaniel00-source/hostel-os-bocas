...
// MEJORA 6: Mostrar Caja (Volunteer y Admin)
...
// MEJORA 6: Finanzas solo para Admin
function showFinances() {
    if (currentProfile?.role !== 'admin') {
        showToast('No tienes permiso para ver finanzas detalladas', 'error');
        return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // MEJORA 3: Fechas de finanzas
    const financeFrom = document.getElementById('finance-date-from');
    const financeTo = document.getElementById('finance-date-to');
    if (financeFrom) financeFrom.value = today;
    if (financeTo) financeTo.value = nextMonth.toISOString().split('T')[0];
...
function showApp() {
    ...
    if (currentProfile?.role === 'admin') {
        ...
        document.getElementById('admin-cash-actions')?.classList.remove('hidden');
        document.getElementById('volunteer-cash-actions')?.classList.add('hidden');
    } else {
        document.querySelectorAll('.admin-only').forEach(el => el.classList.add('hidden'));
        document.getElementById('admin-cash-actions')?.classList.add('hidden');
        document.getElementById('volunteer-cash-actions')?.classList.remove('hidden');
    }
...
// MEJORA 5: Cerrar modal de edición
